...
function updateRecurso($idRedSocial, $recurso)
{
  ...
  if ($recurso == "") {
    ...
    alert("Introduzca el nuevo recurso");
...
function updateCE($ce)
{
  ...
  if ($ce == "") {
    ...
    alert("Introduzca el nuevo Correo Electronico");
...
function updateTel($telefono)
{
  ...
  if ($telefono == "") {
    ...
    alert("Introduzca el nuevo Numero de Telefono");
...

{
  ...
  $consulta = "INSERT INTO carrera(Periodo_Vigencia, Nombre, Recurso, Universidad_ID, Nivel_Educativo_ID, Estatus_ID, RVOE) 

  VALUES ('" . $pVigencia . "', '" . $titulo . "','" . $direccion . "','" . $universidadID . "','" . $seccion . "', '1', '" . $rvoe . "')";
...
function updateOfertas($pVigencia, $Nombre, $direccion, $seccion, $IDoferta, $rvoe)
      ...
      <td>
        ...
        <button type="button" class="btn btn-primary btn-sm"
          ...
          data-toggle="modal" data-target="#ModalUpdateOferta"
           ...
           onclick="mostrareditofertas(`' . $columna1['Periodo_Vigencia'] . '`, `' . $columna1['ID'] . '`,`' . $columna1['Nombre'] . '`,`' . $columna1['Recurso'] . '`,`' . $columna1['secNom'] . '`, `' . $columna1['RVOE'] . '`);">
...
//Obtener tabla con Convocatoria

function obtenerTablaConvocatoria()

{
  ...
  if (mysqli_num_rows($results) > 0) {
    ...
    while ($columna1 = mysqli_fetch_array($results)) {
      console_log("Recurso: '{$columna1['Recurso']}'");
      if ($columna1['Recurso'] != '' && $columna1['Recurso'] != 'NA') {
        echo '
        <form method="POST"> 
          <tr>
          ...
          <td>
            ...
            <button type="button" class="btn btn-primary btn-sm"
              ...
              data-toggle="modal" data-target="#ModalUpdateConvocatoria" onclick="mostrareditConvocatoria(`' . $columna1['Recurso'] . '`)">

              <img src="img/Inicio/editar.svg" class="rounded btn-sm" alt="edit" style="width: 2rem;">

            </button>
            
            <input type="hidden" name="origen" value="Convocatoria">
...
function checkConvocatoria()
{
  ...
  return $aux['Proceso_Admision'] != '' && $aux['Proceso_Admision'] != 'NA';
}
...
function updateCuentaEscolar($claveCT, $nombreArchivo1, $ruta1, $nombreArchivo2, $ruta2, $nombre, $sector)
{
  ...
  if ($nombreArchivo2 != "NA" && $nombreArchivo2 != "" && $ruta2 != "") {
    ...
    $destino2 = "http://feriavirtual.upqroo.edu.mx/SeccionAdministrativa/" . $destino2;
...
function checkOtrosVideos()
{
  //La consulta debe ignorar las seccion 1 (Video de Bienvenida) y 2 (Conferencia)
  $con = conexion10();
...
function ObtenerSeccionesVideos($Idselect)
{
  ...
  if (checkOtrosVideos()) {
    ...
    echo '<option value="4">General</option>';
