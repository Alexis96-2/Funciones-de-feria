<div class="container text-center">
  ...
  <?php
  //Validacion de numero de fotos subidas
  $con = conexion10();
  ...
  if ($_POST['CONFIG'] == "Fotos" && $res['n'] < 8) {
    ...
    //Validacion de numero de videos subidos


    if ($_POST['CONFIG'] == "Videos" && checkOtrosVideos()) {
      echo '<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#ModalAgregarpdf">';
      echo '+ Agregar Video';
      echo '</button>';
    } else if ($_POST['CONFIG'] != "Videos") {
      if ($_POST['CONFIG'] !=  "Convocatoria") {
        echo '<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#ModalAgregarpdf">';
        echo '+ Agregar archivo';
        echo '</button>';
      } else if ($_POST['CONFIG'] ==  "Convocatoria" && !checkConvocatoria()) {
        echo '<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#ModalAgregarpdf">';
        echo '+ Agregar archivo';
        echo '</button>';
      }
    } else if ($_POST['CONFIG'] == "Videos" && !checkOtrosVideos()) {
      echo 'No Puede Subir mas de 6 Videos sin Contar el Video de Bienvenida';
...
<div class="container-fluid text-center">
  ...
  if ($_POST['CONFIG'] == "Convocatoria") {
    ...
    include_once("componentes/AgregarModalConvocatoria.php");

    AgregarModal($_POST['CONFIG']);

    obtenerTablaConvocatoria();

    include_once("componentes/UpdateConvocatoria.php");

    UpdateModalConvocatoria($_POST['CONFIG']);
...
<script type="text/javascript">
  ...
  function mostrareditofertas(pVigencia, id, carrera, archivo, seccion, rvoe)
  ...
  function mostrareditConvocatoria(link)
  ...
  function processSelectedFiles(fileInput) {
    ...
    if (!allowedExtensions.exec(filePath)) {
      alert('Selecione un documento valido');
      fileInput.value = '';
